...
ws=$(hyprctl clients -j | python3 -c "import json,sys; print([c['workspace']['id'] for c in json.load(sys.stdin) if c['class']=='stipple'][0])")
hyprctl dispatch "hl.dsp.focus({ workspace = \"$ws\" })" >/dev/null
sleep 0.3
TIMEOUT=${TIMEOUT:-120} "$(dirname "$0")/tw.sh" "$1"
if [ -n "${SHOT:-}" ]; then
  geom=$(hyprctl clients -j | python3 -c "import json,sys; c=[c for c in json.load(sys.stdin) if c['class']=='stipple'][0]; print(f\"{c['at'][0]},{c['at'][1]} {c['size'][0]}x{c['size'][1]}\")")
  grim -g "$geom" "$SHOT"
fi
