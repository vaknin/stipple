...
function resampleAxis(n, n2) {
  const key = n + '>' + n2;
  let ax = axisCache.get(key);
  if (ax) return ax;
  const start = new Int32Array(n2 + 1), idx = [], wts = [];
  const s = n / n2;
  for (let o = 0; o < n2; o++) {
    start[o] = idx.length;
    const a = o * s, b = Math.min(n, (o + 1) * s);
    if (b - a < 1e-9) { idx.push(Math.min(n - 1, Math.floor(a))); wts.push(1); }
    for (let i = Math.floor(a); i < b && i < n; i++) {
      const w = Math.min(b, i + 1) - Math.max(a, i);
      if (w > 1e-9) { idx.push(i); wts.push(w / (b - a)); }
    }
  }
  start[n2] = idx.length;
  ax = { start, idx: Int32Array.from(idx), wts: Float64Array.from(wts) };
  if (axisCache.size > 16) axisCache.clear();
  axisCache.set(key, ax);
  return ax;
}

export function resampleL(L, W, H, W2, H2) {
  const ax = resampleAxis(W, W2), ay = resampleAxis(H, H2);
  const tmp = new Float64Array(W2 * H);
  for (let y = 0; y < H; y++) {
    const row = y * W;
    for (let x = 0; x < W2; x++) {
      let s = 0;
      for (let t = ax.start[x], e = ax.start[x + 1]; t < e; t++) s += L[row + ax.idx[t]] * ax.wts[t];
      tmp[y * W2 + x] = s;
    }
  }
  const out = new Float32Array(W2 * H2);
  for (let y = 0; y < H2; y++) {
    const t0 = ay.start[y], t1 = ay.start[y + 1];
    for (let x = 0; x < W2; x++) {
      let s = 0;
      for (let t = t0; t < t1; t++) s += tmp[ay.idx[t] * W2 + x] * ay.wts[t];
      out[y * W2 + x] = s;
    }
  }
  return out;
...
export function asciiCells(L, W, H, cols, rows, { method = 'shape', ramp = RAMP, contrast = 1, tune = null } = {}) {
  ...
  const wKey = `${W}|${sx}x${sy}`;
  let circ = weightCache.get(wKey);
  if (!circ) {
    circ = folded ? foldedWeights(sx, sy, W) : circleWeights(SX, SY).map(({ px, wt }) => ({ off: Int32Array.from(px, p => Math.floor(p / SX) * W + (p % SX)), wt }));
    if (weightCache.size > 16) weightCache.clear();
    weightCache.set(wKey, circ);
  }
...
// Circle weights over an sx x sy source cell: circleWeights on the 8 x 17 raster pushed through
// resampleL's area taps (cell edges line up, so every tap stays inside its cell).
function foldedWeights(sx, sy, W) {
  const [SX, SY] = ASCII_SUB;
  const ax = resampleAxis(sx, SX), ay = resampleAxis(sy, SY);
  return circleWeights(SX, SY).map(({ px, wt }) => {
    const acc = new Float64Array(sx * sy);
    for (let t = 0; t < px.length; t++) {
      const x = px[t] % SX, y = Math.floor(px[t] / SX);
      for (let a = ay.start[y]; a < ay.start[y + 1]; a++) {
        for (let b = ax.start[x]; b < ax.start[x + 1]; b++) acc[ay.idx[a] * sx + ax.idx[b]] += wt[t] * ay.wts[a] * ax.wts[b];
      }
    }
    const off = [], w = [];
    for (let i = 0; i < acc.length; i++) if (acc[i] > 0) { off.push(Math.floor(i / sx) * W + (i % sx)); w.push(acc[i]); }
    return { off: Int32Array.from(off), wt: Float64Array.from(w) };
  });
}
