...
import { CROP_DEFAULTS, cropAspect } from './tone.js';
...
export function fitCropFor(crop, w = 1, h = 1) {
  ...
  if (a !== 1) {
    const C = Math.abs(c), S = Math.abs(s);
    zoom = clamp(Math.min(w / a, h) * Math.max((a * C + S) / w, (a * S + C) / h), 1, ZOOM_MAX);
  }
...
export function createCropper({
  ...
  function frame() {
    ...
    const x = Math.round((W - fw) / 2 * dpr) / dpr;
    const y = Math.round((H - side) / 2 * dpr) / dpr;
    return { x, y, size: side, w: fw, cx: x + fw / 2, cy: y + side / 2, W, H, dpr };
  }
  const scale = f => f.size * crop.zoom / Math.min(imgW() / cropAspect(crop), imgH());   // screen px per photo px
  ...
  function draw() {
    ...
    const X = Math.round(f.x * d), Y = Math.round(f.y * d), S = Math.round(f.size * d), SW = Math.round(f.w * d);
    ...
    if (dragging || performance.now() < guidesUntil) {
      ...
      for (const [style, off] of [['rgba(0,0,0,.28)', 1], ['rgba(255,255,255,.72)', 0]]) {
        ctx.beginPath();
        for (const t of [1, 2]) {
          const gx = X + Math.round(SW * t / 3) + 0.5 + off, gy = Y + Math.round(S * t / 3) + 0.5 + off;
          line(gx, Y, gx, Y + S); line(X, gy, X + SW, gy);
        }
        ctx.lineWidth = 1;
  ...
  function fit() {
    const next = fitCrop ? fitCrop(img, snapshot(crop)) : fitCropFor(crop, imgW(), imgH());
