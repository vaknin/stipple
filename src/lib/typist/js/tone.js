...
// and never mutated by their users.
const stageCache = new WeakMap();
function memo(img, key, fn) {
  let m = stageCache.get(img);
  if (!m) { m = new Map(); stageCache.set(img, m); }
  let v = m.get(key);
  if (v === undefined) {
    v = fn();
    m.set(key, v);
...
export function cropAspect(crop) {
  const a = +(crop && crop.aspect);
  return a > 0 && Number.isFinite(a) ? a : 1;
}

/** [width, height] of the crop in source pixels (both cropSide for a square crop). */
export function cropSize(width, height, crop) {
  const a = cropAspect(crop);
  if (a === 1) { const s = cropSide(width, height, crop); return [s, s]; }
  const h = Math.min(width / a, height) / Math.max(0.05, (crop && crop.zoom) || 1);
  return [h * a, h];
}
...
function toneSoft(img, t, { target, boost, stretch }) {
  const { W, H } = img, N = W * H;
  const wts = centreWeights(W, H);
  const lv = memo(img, `soft|${t.auto ? 1 : 0}|${stretch}`, () => {
    const o = new Float32Array(N);
    const [a, b] = levelled(img, t, wts, stretch, o);
    return { lo: a, hi: b, out: o, base: null };
  });
  const out = Float32Array.from(lv.out);
  const { lo, hi } = lv;
  ...
  if (detail > 0) {
    if (!lv.base) {
      lv.base = Float32Array.from(lv.out);
      boxBlur(lv.base, W, H, boxRadiusForSigma(Math.max(1, 0.025 * Math.max(W, H))));
    }
    const base = lv.base;
    const k = detail * 1.8;
...
function toneTexture(img, t, { target, boost, stretch }) {
  ...
  const eq = W >= 4 && H >= 4 ? clahe(out, new Float32Array(N), W, H,
    Math.max(1, Math.round(P.tiles * W / long)), Math.max(1, Math.round(P.tiles * H / long)), P.clip, P.bins) : null;
  const wts = new Float32Array(N);
  for (let i = 0; i < N; i++) wts[i] = cw[i] * (1 - P.salient + P.salient * mask[i]);
  return { lo, hi, out, mask, eq, wts };
  });
  const { lo, hi, mask, eq, wts } = pre;

  const dScale = t.detail / 0.35;
  const amount = Math.min(1, P.amount * Math.min(1.4, dScale) * (1 + 0.15 * boost));
  const out = Float32Array.from(memo(img, `texture|${t.auto ? 1 : 0}|${stretch}|${t.detail}|${boost}`, () => {
  const out = Float32Array.from(pre.out);
  if (amount > 0 && W >= 4 && H >= 4) {
...
function toneSketch(img, t, { target, boost, stretch }) {
  ...
  const ink = t.detail > 0 ? xdogFrom(blurs, W, H, lp) : new Float32Array(N);
...
function tonePoster(img, t, { stretch }) {
  ...
  const pre = memo(img, `poster|${t.auto ? 1 : 0}|${stretch}`, () => {
    const o = new Float32Array(N);
    const [a, b] = levelled(img, t, wts, stretch, o);
    const sm0 = Float32Array.from(o);
    boxBlur(sm0, W, H, 1, 1);
    const base = Float32Array.from(sm0);
    boxBlur(base, W, H, boxRadiusForSigma(Math.max(1, 0.02 * Math.max(W, H))));
    return { lo: a, hi: b, sm0, base };
  });
  const { lo, hi } = pre;
  ...
  const sm = Float32Array.from(pre.sm0);
  const dScale = t.detail / 0.35;
  if (dScale > 0) {
    const base = pre.base;
