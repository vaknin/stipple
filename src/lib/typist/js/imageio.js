...
export function autoCrop(img, aspect = 1) {
  ...
  if (alphaBox) {
    ...
    const cropH = Math.max(bh, bw / a) * 1.12;
    return {
      x: (alphaBox.x0 + alphaBox.x1) / 2, y: (alphaBox.y0 + alphaBox.y1) / 2,
      zoom: Math.max(0.35, Math.min(6, Math.min(w / a, h) / cropH)), rotation: 0,
    };
  }
  const portrait = h > w * 1.15;
  if (a === 1) return { x: 0.5, y: portrait ? Math.max(0.5 * w / h, 0.42) : 0.5, zoom: 1, rotation: 0 };
  return { x: 0.5, y: portrait ? Math.max(Math.min(w / a, h) / (2 * h), 0.42) : 0.5, zoom: 1, rotation: 0 };
}
